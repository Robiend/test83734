...
if( ! class_exists( 'SMMS_Gutenberg' ) ){

	class SMMS_Gutenberg{
		/**
		 ...
		 */
		private $_category_slug = 'smms-blocks';
		...
		/**
		 * SMMS_Gutenberg constructor.
		 */
		private function __construct() {
			add_action( 'init', array( $this, 'register_blocks' ), 30 );
			add_action( 'enqueue_block_editor_assets', array( $this, 'enqueue_block_editor_assets' ) );
			add_action( 'wp_ajax_smms_plugin_fw_gutenberg_do_shortcode', array( $this, 'do_shortcode' ) );
			add_action( 'wc_ajax_smms_plugin_fw_gutenberg_do_shortcode', array( $this, 'do_shortcode' ) );
		}
		...
		public function enqueue_block_editor_assets() {
			$ajax_url = function_exists( 'WC' ) ? add_query_arg( 'wc-ajax', 'smms_plugin_fw_gutenberg_do_shortcode', trailingslashit( site_url() ) ) : admin_url( 'admin-ajax.php' );
			$suffix   = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
			$deps     = apply_filters( 'smms_plugin_fw_gutenberg_script_deps', array(
				'wp-blocks',
				'wp-element',
				'smms-js-md5'
			) );
			wp_register_script( 'smms-js-md5', SMM_CORE_PLUGIN_URL . '/assets/js/javascript-md5/md5.min.js', array(), '2.10.0', true );
			wp_enqueue_script( 'smms-gutenberg', SMM_CORE_PLUGIN_URL . '/assets/js/smms-gutenberg' . $suffix . '.js', $deps, smms_plugin_fw_get_version(), true );
			wp_localize_script( 'smms-gutenberg', 'smms_gutenberg', $this->_blocks_args );
			wp_localize_script( 'smms-gutenberg', 'smms_gutenberg_ajax', array( 'ajaxurl' => $ajax_url ) );
		}
		...
		public function register_blocks(){
			...
			foreach (  $this->_to_register_blocks as $block => $args ){
				...
				if( register_block_type( "smms/{$block}", $block_args ) ){
		...
		public function block_category( $categories, $post ){
			return array_merge(
				...
				array(
					array(
						'slug' => 'smms-blocks',
						'title' => _x( 'SMMS', '[gutenberg]: Category Name', 'smms-plugin-fw' ),
					),
				)
			);
		}
		...
		public function do_shortcode(){
			...
			$sc             = ! empty( $_POST['shortcode'] ) ? sanitize_text_field($_POST['shortcode']) : '';

			if( ! apply_filters( 'smms_plugin_fw_gutenberg_skip_shortcode_sanitize', false ) ){
				$sc = sanitize_text_field( stripslashes( $sc ) );
			}

			do_action( 'smms_plugin_fw_gutenberg_before_do_shortcode', $sc, $current_action );
			_e( do_shortcode( apply_filters( 'smms_plugin_fw_gutenberg_shortcode', $sc, $current_action ) ));
			do_action( 'smms_plugin_fw_gutenberg_after_do_shortcode', $sc, $current_action );
...
if ( ! function_exists( 'SMMS_Gutenberg' ) ) {
	/**
	 * Main instance of plugin
	 *
	 * @return SMMS_Gutenberg
	 * @since  1.0
	 * @author sam softnwords
	 */
	function SMMS_Gutenberg() {
		return SMMS_Gutenberg::instance();
	}
}

...
class SMM_CPT_Unlimited {
    ...
    public function register_post_type() {
        $labels = array(
            ...
            'add_new'            => sprintf( __( 'Add %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'add_new_item'       => sprintf( __( 'Add New %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'edit_item'          => sprintf( __( 'Edit %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'new_item'           => sprintf( __( 'New %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'all_items'          => sprintf( __( 'All %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'view_item'          => sprintf( __( 'View %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ),
            'search_items'       => sprintf( __( 'Search %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found'          => sprintf( __( 'No %s found', 'smms-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'smms-plugin-fw' ), ucfirst( $this->_labels['plural'] ) ),
            'parent_item_colon'  => '',
    ...
    public function register_cptu_post_types() {
        ...
        foreach ( $post_types as $pt ) {
            ...
            $labels = array(
                ...
                'add_new'            => sprintf( __( 'Add %s', 'smms-plugin-fw' ), $label_singular ),
                'add_new_item'       => sprintf( __( 'Add New %s', 'smms-plugin-fw' ), $label_singular ),
                'edit_item'          => sprintf( __( 'Edit %s', 'smms-plugin-fw' ), $label_singular ),
                'new_item'           => sprintf( __( 'New %s', 'smms-plugin-fw' ), $label_singular ),
                'all_items'          => sprintf( __( 'All %s', 'smms-plugin-fw' ), $label_plural ),
                'view_item'          => sprintf( __( 'View %s', 'smms-plugin-fw' ), $label_singular ),
                'search_items'       => sprintf( __( 'Search %s', 'smms-plugin-fw' ), $label_plural ),
                'not_found'          => sprintf( __( 'No %s found', 'smms-plugin-fw' ), $label_plural ),
                'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'smms-plugin-fw' ), $label_plural ),
                'parent_item_colon'  => '',
                'menu_name'          => $title
            );
            ...
            if ( $this->_args['has_taxonomy'] && ! empty( $taxonomy ) ) {
                ...
                $labels = array(
                    'name'              => sprintf( _x( '%s Categories', 'taxonomy general name', 'smms-plugin-fw' ), $label_singular ),
                    'singular_name'     => _x( 'Category', 'taxonomy singular name', 'smms-plugin-fw' ),
                    'search_items'      => __( 'Search Categories', 'smms-plugin-fw' ),
                    'all_items'         => __( 'All Categories', 'smms-plugin-fw' ),
                    'parent_item'       => __( 'Parent Category', 'smms-plugin-fw' ),
                    'parent_item_colon' => __( 'Parent Category:', 'smms-plugin-fw' ),
                    'edit_item'         => __( 'Edit Category', 'smms-plugin-fw' ),
                    'update_item'       => __( 'Update Category', 'smms-plugin-fw' ),
                    'add_new_item'      => __( 'Add New Category', 'smms-plugin-fw' ),
                    'new_item_name'     => __( 'New Category Name', 'smms-plugin-fw' ),
                    'menu_name'         => __( 'Category', 'smms-plugin-fw' ),
                );
    ...
    public function add_cptu_menu_item() {
        ...
        if ( strpos( $post_type, $this->_prefix_cpt ) !== false ) {
            ...
            $submenu[ $item ][20] = array( sprintf( __('Add %s', 'smms-plugin-fw'), ucfirst( $label_singular ) ), 'edit_posts', 'post-new.php?post_type=' . $post_type );

            global $wp_taxonomies;
            $taxonomy = get_post_meta( $portfolio->ID, '_taxonomy', true );
            if ( isset( $wp_taxonomies[ $taxonomy ] ) ) {
                $submenu[ $item ][25] = array( __('Categories', 'smms-plugin-fw'), 'edit_posts', 'edit-tags.php?taxonomy=' . $taxonomy . '&post_type=' . $post_type );
            }
        }
    ...
    public function add_metabox_cptu() {
        ...
        $metabox_args = array(
            'label'    => sprintf( __( '%s Settings', 'smms-plugin-fw' ), $this->_labels['singular'] ),
            'pages'    => $this->_name, //or array( 'post-type1', 'post-type2')
            'context'  => 'normal', //('normal', 'advanced', or 'side')
            'priority' => 'default',
            'tabs'     => array(
                'settings' => array(
                    'label'  => __( 'Settings', 'smms-plugin-fw' ),
                    'fields' => apply_filters( 'smm_cptu_fields', array(
                        'type'      => array(
                            'label' => __( 'Type', 'smms-plugin-fw' ),
                            'desc'  => sprintf( __( 'Layout for this %s' , 'smms-plugin-fw' ), strtolower( $this->_labels['singular'] ) ),
                            'type'  => 'select',
                            'options' => isset( $layouts ) ? $layouts : array(),
                            'std'   => '' ),

                        'rewrite'     => array(
                            'label' => __( 'Rewrite', 'smms-plugin-fw' ),
                            'desc'  => __( 'Univocal identification name in the URL for each product (slug from post if empty)', 'smms-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_singular' => array(
                            'label' => __( 'Label in Singular', 'smms-plugin-fw' ),
                            'desc'  => __( 'Set a label in singular (title of portfolio if empty)', 'smms-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'label_plural' => array(
                            'label' => __( 'Label in Plural', 'smms-plugin-fw' ),
                            'desc'  => __( 'Set a label in plural (title of portfolio if empty)', 'smms-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy' => array(
                            'label' => __( 'Taxonomy', 'smms-plugin-fw' ),
                            'desc'  => __( 'If you want to use categories in the portfolio, set a name for taxonomy. Name should be a slug (must not contain capital letters nor spaces) and must not be more than 32 characters long (database structure restriction).', 'smms-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'taxonomy_rewrite' => array(
                            'label' => __( 'Taxonomy Rewrite', 'smms-plugin-fw' ),
                            'desc'  => __( 'Set univocal name for each category page URL.', 'smms-plugin-fw' ),
                            'type'  => 'text',
                            'std'   => '' ),

                        'single_layout' => array(
                            'label' => __( 'Single layout', 'smms-plugin-fw' ),
                            'desc'  => __( 'Layout for single page of this portfolio', 'smms-plugin-fw' ),
                            'type'  => 'select',
        ...
        if ( $this->_args['manage_layouts'] ) {
            ...
            foreach ( $this->layouts as $layout => $args ) {
                ...
                $metabox_args['tabs']['settings']['fields'][ $layout . '_title' ] = array(
                    'desc' => $args['name'] . ' ' . __( 'layout settings', 'smms-plugin-fw' ),
                    'type' => 'title',
                    'deps' => array(
                        'ids' => '_type',
                        'values' => $layout
    ...
    public function add_quick_links_metaboxes() {
        // CPTU
        add_meta_box( $this->_name . '_quick_links', __( 'Quick links', 'smms-plugin-fw' ), array( $this, 'quick_links_cptu_inner' ), $this->_name, 'side', 'high' );
        ...
        foreach ( $post_types as $post ) {
            ...
            add_meta_box( $post->post_type . '_quick_links', __( 'Quick links', 'smms-plugin-fw' ), array( $this, 'quick_links_cpt_inner' ), $post_type, 'side', 'high' );
        }
    }
    ...
    public function quick_links_cptu_inner( $post ) {
        extract( $this->_cpt_args( $post ) );
        ?>
        <a href="<?php _e( admin_url( 'edit.php?post_type=' . get_post_meta( $post->ID, '_post_type', true ) )) ?>"><?php printf( __( 'View %s', 'smms-plugin-fw' ), $label_plural ) ?></a>
        <?php
    }
    ...
    public function quick_links_cpt_inner( $post ) {
        ...
        ?>
        <a href="<?php _e( admin_url( "post.php?post={$post->ID}&action=edit" ) )?>"><?php printf( __( 'Edit %s', 'smms-plugin-fw' ), $post->post_title ) ?></a>
        <?php
    }
    ...
    public function cptu_change_columns( $column, $post_id ) {
        ...
        switch ( $column ) {
            case 'actions' :
                _e( '<a href="' . admin_url( "post.php?post={$post_id}&action=edit" ) . '" class="button-secondary">' . sprintf( __( 'Edit %s', 'smms-plugin-fw' ), ucfirst( $this->_labels['singular'] ) ) . '</a> ');
                _e( '<a href="' . admin_url( 'edit.php?post_type=' . get_post_meta( $post_id, '_post_type', true ) ) . '" class="button-secondary">' . sprintf( __( 'View %s', 'smms-plugin-fw' ), $label_plural ) . '</a> ');
                break;
        }
    }
    ...
    public function add_shortcode_to_box( $shortcodes ) {
        ...
        $args = array(
            $this->_args['shortcode_name'] => array(
                ...
                'description' => sprintf( __( 'Show frontend of the %s', 'smms-plugin-fw' ), $this->_labels['main_name'] ),
                'tab'         => 'cpt',
                'create'      => false,
                'has_content' => false,
    ...
    */
    ...
    public function admin_assets() {
        wp_enqueue_media();
        wp_enqueue_script( 'smm-cptu', SMM_CORE_PLUGIN_URL . '/assets/js/smm-cpt-unlimited.js', array('jquery'), '', true );
    }
        ...
        <script>
            (function($) {
                ...
                var button = $('<a />', {
                    ...
                    'data-uploader_title': '<?php printf( __( 'Add %s from images', 'smms-plugin-fw' ), $label_plural ) ?>',
                    'data-uploader_button_text': '<?php printf( __( 'Add %s', 'smms-plugin-fw' ), $label_plural ) ?>'
                }).text('<?php _e( 'Upload multiple files', 'smms-plugin-fw' ) ?>');
...
//        }
    ...
    public function archive_template_loop() {
        _e( $this->add_shortcode( array( 'name' => $GLOBALS['wp_query']->post->post_type ) ));
    }

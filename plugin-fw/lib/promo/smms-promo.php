					...
					if( $show_promo && ! empty( $start_date ) && ! empty( $end_date ) ){
						...
						if( $end_date >= $start_date && $now >= $start_date && $now <= $end_date ){
							...
							if ( true === $show_notice ) :
								...
								?>
                                <div id="<?php _e( $unique_promo_id) ?>" class="smms-notice-is-dismissible notice notice-smms notice-alt is-dismissible" style="<?php _e( $style)?>" data-expiry= <?php _e( $promo->end_date) ?>>
                                    <p>

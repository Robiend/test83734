    ...
    class SMM_Plugin_Panel {
        ...
        public function __construct( $args = array() ) {
            ...
            if ( !empty( $args ) ) {
                ...
                $default_args = array(
                    ...
                    'page_title'  => __( 'Plugin Settings', 'smms-plugin-fw' ),
                    'menu_title'  => __( 'Settings', 'smms-plugin-fw' ),
        ...
        public function add_menu_page() {
            ...
            if ( !isset( $admin_page_hooks[ 'smms_plugin_panel' ] ) ) {
                ...
                if ( !!$show ) {
                    add_menu_page( 'smms_plugin_panel', 'SMMS', $capability, 'smms_plugin_panel', null, smms_plugin_fw_get_default_logo(), $position );
        ...
        public function add_premium_version_upgrade_to_menu() {
            ...
            if ( apply_filters( 'smm_show_upgrade_to_premium_version', isset( $submenu[ 'smms_plugin_panel' ] ) ) ) {
                $submenu[ 'smms_plugin_panel' ][ 'how_to' ] = array(
                    sprintf( '%s%s%s', '<span id="smms-how-to-premium">', __( 'How to install premium version', 'smms-plugin-fw' ), '</span>' ),
                    'install_plugins',
                    SMMS_SMAPI_PREMIUM_SUPPORT.'smm-api',
                    __( 'How to install premium version', 'smms-plugin-fw' ),
                );
            }
        }
        ...
        <div class="wrap <?php _e( $wrap_class) ?>">
            ...
            <div id="wrap" class="smms-plugin-fw plugin-option smm-admin-panel-container">
                ...
                <div class="<?php _e( $panel_content_class) ?>">
                    <h2><?php _e( $this->get_tab_title()) ?></h2>
                    <?php if ( $this->is_show_form() ) : ?>
                        <form id="smms-plugin-fw-panel" method="post" action="options.php">
                            ...
                            <input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'smms-plugin-fw' ) ?>" style="float:left;margin-right:10px;"/>
                        </form>
                        <form method="post">
                            ...
                            <input type="submit" name="smm-reset" class="button-secondary" value="<?php _e( 'Reset to default', 'smms-plugin-fw' ) ?>"
        ...
        function get_current_tab() {
            ...
            if ( isset( $_REQUEST[ 'smm_tab_options' ] ) ) {
                return $_REQUEST[ 'smm_tab_options' ];
        ...
        public function message() {
            ...
            $message = array(
                'element_exists'   => $this->get_message( '<strong>' . __( 'The element you have entered already exists. Please, enter another name.', 'smms-plugin-fw' ) . '</strong>', 'error', false ),
                'saved'            => $this->get_message( '<strong>' . __( 'Settings saved', 'smms-plugin-fw' ) . '.</strong>', 'updated', false ),
                'reset'            => $this->get_message( '<strong>' . __( 'Settings reset', 'smms-plugin-fw' ) . '.</strong>', 'updated', false ),
                'delete'           => $this->get_message( '<strong>' . __( 'Element deleted correctly.', 'smms-plugin-fw' ) . '</strong>', 'updated', false ),
                'updated'          => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'smms-plugin-fw' ) . '</strong>', 'updated', false ),
                'settings-updated' => $this->get_message( '<strong>' . __( 'Element updated correctly.', 'smms-plugin-fw' ) . '</strong>', 'updated', false ),
                'imported'         => $this->get_message( '<strong>' . __( 'Database imported correctly.', 'smms-plugin-fw' ) . '</strong>', 'updated', false ),
                'no-imported'      => $this->get_message( '<strong>' . __( 'An error has occurred during import. Please try again.', 'smms-plugin-fw' ) . '</strong>', 'error', false ),
                'file-not-valid'   => $this->get_message( '<strong>' . __( 'The added file is not valid.', 'smms-plugin-fw' ) . '</strong>', 'error', false ),
                'cant-import'      => $this->get_message( '<strong>' . __( 'Sorry, import is disabled.', 'smms-plugin-fw' ) . '</strong>', 'error', false ),
                'ord'              => $this->get_message( '<strong>' . __( 'Sorting successful.', 'smms-plugin-fw' ) . '</strong>', 'updated', false )
            );
	    ...
	    public function add_smms_ui( $field ) {
		    ...
		    if ( 'admin.php' === $pagenow && strpos( get_current_screen()->id, $this->settings['page'] ) !== false ) {
			    switch ( $field['type'] ) {
				    case 'datepicker':
					    _e('<span class="smms-icon icon-calendar"></span>');
					    break;
				    default:
					    break;
			    }

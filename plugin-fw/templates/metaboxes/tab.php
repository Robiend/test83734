...
<div class="smms-plugin-fw metaboxes-tab <?php _e(  esc_attr($classes ))?>">
    ...
    <ul class="metaboxes-tabs clearfix"<?php if ( count( $tabs ) <= 1 ) : ?> style="display:none;"<?php endif; ?>>
        ...
        foreach ( $tabs as $key=>$tab ) :
            ...
            ?>
        <li id="<?php _e( $anchor_id) ?>" <?php if ( !$i ) : ?>class="tabs"<?php endif ?> <?php _e( smms_field_deps_data( $tab )) ?>>
            <a href="#<?php _e( urldecode( $key )) ?>"><?php _e( $tab[ 'label' ]) ?></a></li><?php
            $i++;
        endforeach;
        ?>
    </ul>
    ...
    <?php foreach ( $tabs as $key=> $tab ) :
        ...
        ?>
        <div class="tabs-panel" id="<?php _e( urldecode( $key )) ?>">
            <?php
            if ( !isset( $tab[ 'fields' ] ) ) {
                continue;
            }
            ...
            foreach ( $tab[ 'fields' ] as $id_tab => $field ) :
                ...
                ?>
                <div class="<?php _e( $container_classes) ?>">
                    <?php

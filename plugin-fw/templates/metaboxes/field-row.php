...
?>
<div id="<?php _e(  $field[ 'id' ]) ?>-container" <?php _e( smms_field_deps_data( $field )) ?> class="smms-plugin-fw-metabox-field-row">
    <?php if ( $display_field_only ) :
        smms_plugin_fw_get_field( $field, true );
    else: ?>
        <label for="<?php _e(  $field[ 'id' ]) ?>"><?php _e(  $field[ 'title' ]) ?></label>
        <?php smms_plugin_fw_get_field( $field, true ); ?>
        <div class="clear"></div>
        <span class="description"><?php  _e(  $field[ 'desc' ]) ?></span>
    <?php endif; ?>
</div>
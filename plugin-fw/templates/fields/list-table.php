...
<div id="<?php _e( $id )?>" class="smms-plugin-fw-list-table <?php _e( $class) ?>">
        <div class="smms-plugin-fw-list-table-container smms-plugin-fw smm-admin-panel-container">
            <div class="list-table-title">
            <h2>
		        <?php _e( isset( $title ) ? $title : '' )?>
            </h2>
	        <?php if( $show_button ): ?>
                <a href="<?php _e( $add_new_url) ?>" class="smms-add-button">
			        <?php _e( $add_new_button) ?>
                </a>
	        <?php endif ?>
            </div>

	        <?php if( isset( $desc) && !empty( $desc ) ) :?>
                <p class="smms-section-description"><?php _e( $desc) ?></p>
			<?php
            endif;

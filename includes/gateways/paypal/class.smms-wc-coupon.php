...
//****************************************************************Trigger API order based on coupon codes
        ...
        function smm_api_order_process( $mypostArraynumber, $txn_id, $order_is_paid )
	{
		...
		// for testing purpose
               ...
               $my_ipn_query = "UPDATE {$wpdb->posts}	SET post_excerpt ='%s'
               WHERE ID ='%d'";

               $wpdb->query($wpdb->prepare($my_ipn_query, $txn_id, $mypostArraynumber));
...
//Save error in log text in plugin folder
               ...
               file_put_contents(__DIR__.'/smm_log.txt', $my_ipn_query_error);
...
//function call to extract item details from order
               $data_post1 = my_smm_order_details_data($data_post1);
				...
				$wpdb->prepare("SELECT * FROM {$db_table_name} WHERE my_site_item=%s",$customer_order_Item));
...
// return order id or Error
	...
	// Record API server string data for other reasons
				...
				update_post_meta( $mypostArraynumber, 'API_STAT', json_encode($sam_order)
				.' Task: Correct response parameter or server response');
				// Record curl error message
				if($sam_order->curl_errno != "")
	update_post_meta( $mypostArraynumber, 'API_STAT', json_encode($sam_order).' Reason: API data is not received from server');
				}
				else
				{
				$sam_apiobj = json_decode(json_encode($sam_order),true);// converting object into array

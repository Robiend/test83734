	...
	class SMAPI_Subscription_Paypal {
		...
		public function subscription_args( $args ) {
			...
			if ( count( $item_names ) > 1 ) {
				$args['item_name'] = $this->format_item_name( sprintf( __( 'Order %s', 'smm-api' ), $order->get_order_number() . " - " . implode( ', ', $item_names ) ) );
			} else {
		...
		function change_subscription_status( $subscriber_id , $status  ) {
			...
			$api_request = array('USER' => urlencode( $this->api_username ),
			                ...
			                'NOTE' =>  urlencode( sprintf( __( 'Subscription %s at %s', 'smm-api' ), strtolower( $status ), get_bloginfo( 'name' ) ) ));

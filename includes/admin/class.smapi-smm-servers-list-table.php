...
class SMMS_SMAPI_Servers_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        GROUP BY smapi_p.ID $order_string", $this->post_type
        );

        $totalitems = $wpdb->query( $query );
//echo serialize($totalitems).$query; // FOR TESTING
   ...
   function get_bulk_actions(  ) {
    ...
    function column_cb($item) {
        return sprintf(
            '<input type="checkbox" name="smapi_server_ids[]" value="%s" />',  $item->ID
        );
    }
    ...
    function column_id($item) {
        $actions = array(
            'edit'      => sprintf('<a class="%s" href="%s">Edit</a>','server_edit', '#'.$item->ID),

            'delete'    => sprintf('<a class="%s" href="%s">Delete</a>','server_delete', '#'.$item->ID),
        );

...
class SMMS_SMAPI_SMM_API_ITEMS_List_Table extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        // API ITEM LISTING
        $query = $wpdb->prepare( "SELECT smapi_pm.* FROM " .$wpdb->prefix ."postmeta as smapi_pm INNER JOIN $wpdb->posts as smapi_p ON ( smapi_pm.post_id = '$this->smm_api_server_item' )
        WHERE 1=1 $link
        AND smapi_p.post_type = %s
        AND smapi_pm.meta_key LIKE %s
        GROUP BY smapi_pm.meta_id $order_string", $this->post_type, $this->meta_key
        );
...
//echo $query; // FOR TESTING
        ...
        $smm_api_items = $wpdb->get_results( $query );
...
//echo serialize($this->api_servers);
    ...
    function column_default( $item, $column_name ) {
        ...
        switch( $column_name ) {
            ...
            case 'api_item_status':
                ...
                return $api_item_status;
	            
	            break;
	       case 'api_item_subscription':
                $api_item_subscription = $smm_api_items['f_item_subscribe_check'];
                return $api_item_subscription;
                break;
	       case 'api_item_display':
               return  '<a href=#'.$item->meta_key.' id="more_'.$item->meta_key.'">More</a>';
                break;
    ...
    function process_bulk_action(  ) {
        ...
        if( !empty( $actions) && isset($_POST['smapi_meta_key'] )){
            ...
            $smm_items = array_map( 'esc_attr', $_POST['smapi_meta_key'] );
            ...
            if( $actions == 'delete' ){
                ...
                global $wpdb;
                $PostIDs = $wpdb->get_results( "SELECT DISTINCT (post_id) 
                        FROM {$wpdb->prefix}postmeta
                        WHERE post_id IN (SELECT post_id 
                        FROM {$wpdb->prefix}postmeta 
                        WHERE meta_value = $current_selection) 
                        And post_id IN (SELECT post_id 
                        FROM {$wpdb->prefix}postmeta 
                        WHERE meta_value = '$item_meta_key')", ARRAY_A );
    ...
    function column_cb($item) {
        return sprintf(
            '<input type="checkbox" name="smapi_meta_key[]" value="%s" />',  $item->meta_key
        );
    }
 ...
 function Smm_item_form() {
     ...
     $server_query = $wpdb->prepare( "SELECT smapi_p.post_title, smapi_p.ID FROM $wpdb->posts as smapi_p INNER JOIN " . $wpdb->prefix . "postmeta as smapi_pm ON ( smapi_p.ID = smapi_pm.post_id )
        ...
        $this->total_servers = $wpdb->query( $server_query );
        $this->api_servers  = $wpdb->get_results( $server_query, ARRAY_A );

         _e( '<div id="icon-tools" class="icon32"></div>');

         ?>
            <h1> AVAILABLE API ITEMS FOR EACH API SERVER 
			...
			_e($api_server_list_options_output.'</select>');
			//echo $this->select_order;
			?>
			 </h1>
	    ...
	    <caption>API ITEM</caption>
            ...
            <input type="hidden" name="f_post_id" value="<?=$this->smm_api_server_item?>">

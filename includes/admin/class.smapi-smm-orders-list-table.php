   ...
   function get_bulk_actions(  ) {
    ...
    function process_bulk_action(  ) {
        ...
        if( !empty( $actions) && isset($_POST['smapi_order_ids'] )){
            ...
            $orders = (array) $_POST['smapi_order_ids'];
    ...
    function column_cb($item) {
        return sprintf(
            '<input type="checkbox" name="smapi_order_ids[]" value="%s" />',  $item['api_order']
        );
    }
    ...
    function column_api_order($item) {
        $actions = array(
            ...
            'delete'    => sprintf('<a class="%s" href="%s">Delete</a>','order_delete', '#'.$item['api_order']),
        );
    ...
    foreach($order->get_items() as $item_id => $item){
       ...
       if($product):
        ...
        // Order Item data (unprotected on Woocommerce 3)
             ...
             if( $api_server_order = $order->get_meta( 'Response' ))
			 {
			$api_server_order =	 substr($api_server_order, 0, strpos($api_server_order, "received"));
			$api_server_order_obj = json_decode($api_server_order);
            ...
            //file_put_contents(plugin_dir_path( __FILE__ )."check.txt", "This is from admin/orderlisttable.php line 284 order - ".$api_server_order_obj->order.PHP_EOL,FILE_APPEND);

...
if ( !class_exists( 'SMAPI_Subscription_Cron' ) ) {
    ...
    class SMAPI_Subscription_Cron {
	    ...
	    public function __construct() {
	    	...
	    	//add_action( 'wp_loaded', array( $this, 'renew_orders' ), 30 );
	    ...
	    public function set_cron() {
            ...
            $time_start = current_time('timestamp') + 3600;//execute every hour
            $next_event = wp_get_scheduled_event( 'smapi_renew_orders' );
            if($next_event->timestamp < current_time('timestamp')+3600)
	    ...
	    public function renew_orders() {
		    ...
		    $query = $wpdb->prepare( "SELECT ID,post_content,post_title,post_excerpt FROM {$wpdb->prefix}posts 
		         WHERE post_type  = %s 
		    ...
		    $subscriptions = $wpdb->get_row( $query );
	    ...
	    public function smapi_trash_pending_subscriptions() {
		    ...
		    $subscriptions = $wpdb->get_results( $wpdb->prepare( "SELECT smapi_p.ID FROM {$wpdb->prefix}posts as smapi_p
                ...
                ", 'smapi_subscription', date( 'Y-m-d H:i:s', $time ) ) );

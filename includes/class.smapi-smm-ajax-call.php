...
class SMMS_Ajax_Call_Admin {
		...
		public function smm_ajax_action_function_server_display(){ 
			...
			$query = $wpdb->prepare( "SELECT smapi_p.* FROM $wpdb->posts as smapi_p INNER JOIN " . $wpdb->prefix . "postmeta as smapi_pm ON ( smapi_p.ID = smapi_pm.post_id )
			WHERE ID='$post_id' $link
			AND smapi_p.post_type = %s
			GROUP BY smapi_p.ID $order_string", $post_type
			);
		...
		public function smm_ajax_action_function_server_save(){
            	...
            	if(isset($_POST) && count($_POST))
         			...
         			$missing = array();
         			foreach ($_POST as $key => $value) { if ($key != "fsid" && $value == "") { array_push($missing, $key);}}
                 	if (count($missing) > 0) {
                 	$response['success'] = 0;
		...
		public function smm_ajax_action_function_item_delete(){
			...
			$post_id  = get_option('smm_api_server_item');
                ...
                $PostIDs = $wpdb->get_results( "SELECT DISTINCT (post_id) 
                        ...
                        FROM {$wpdb->prefix}postmeta 
                        WHERE meta_value = '$smm_item_delete')", ARRAY_A );

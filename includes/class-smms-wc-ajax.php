...
class SMM_WC_AJAX extends WC_AJAX {
	...
	public static function smm_save_variations() {
		...
		if ( $errors ) {
			_e( '<div class="error notice is-dismissible">');

			foreach ( $errors as $error ) {
				_e( '<p>' . wp_kses_post( $error ) . '</p>');
			}

			_e( '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' ). esc_html__( 'Dismiss this notice.', 'woocommerce' ) . '</span></button>';
			_e( '</div>');

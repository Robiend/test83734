 ...
 */
		public function smm_cfwc_add_custom_data_to_order( $item, $cart_item_key, $values, $order ) {
			...
			$item->add_meta_data( __( 'Entered', 'smm-api' ), $values['smm-cfwc-title-field'], true );
			...
			foreach ( $order_items as $key => $order_item ) { 
				...
				if ( SMMS_WC_Subscription()->is_subscription( $id ) ) {
					...
					if ( $subscription->id ) {
						...
						$order->add_order_note( sprintf( __( 'A new subscription #%d has been created from this order', 'smm-api' ), $subscription->id ) );
                            ...
                            foreach($variation_attributes as $attribute_taxonomy => $term_slug ){
	                            ...
	                            $order_item_meta_obj = json_decode($order_item_meta); 
	                                ...
	                                // API call based on server name as post ID
                                            ...
                                            $message = __( json_encode($Order_response).'received for product', 'smm-api' );
...
*/
			...
			if ( !$item_id ) {
				throw new Exception( sprintf( __( 'Error %d: unable to create the order. Please try again.','smm-api'), 402 ) );
			} else {
...
*/
			...
			$order->add_order_note( sprintf( __( 'This order has been created to renew subscription #%s', 'smm-api' ), admin_url( 'post.php?post=' . $subscription->id . '&action=edit' ), $subscription->id));
		...
		public function renew_order_old( $subscription_id ) {
			...
			$order->add_order_note( sprintf( __( 'This order has been created to renew the subscription #%d', 'smm-api' ), $subscription_id ) );

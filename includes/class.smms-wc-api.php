...
class SMAPI_Api{
	...
	public function services() { // get services
      ...
      private function connect($post) {
          ...
          $get_details_of_request = "server arg = ".$this->api_server_url."?"." server url = ".$smm_server_url." return data = ".serialize($httpcode)."body data = ";
        // file_put_contents(plugin_dir_path( __FILE__ )."check.txt", $get_details_of_request );
                if ( is_wp_error( $response ) ) {
                $error_message =  $httpcode->get_error_message();
                $myarr = array('Error from API HTTP CALL' => $error_message, 'result' => $get_details_of_request );

                $result = json_encode($myarr, JSON_FORCE_OBJECT);
                }
...
class BCDE { 
        public $cb  = "";
        public $SERVICEID  = "1";
        public $DESCRIPTION =  "Director";
        public $RATE =     "$5,300";
        public $MIN =  "2011/07/25";
        public $MAX =     "Edinburgh";
        public $STATUS =      "5421";
        public $SUBSCRIPTION = "yes";
        public $VIEW = "more";
} 
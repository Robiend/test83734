...
if ( !class_exists( 'SMMS_WC_Subscription_Admin' ) ) {
    ...
    class SMMS_WC_Subscription_Admin {
         ...
         */
        protected $_premium_landing = 'http://softnwords.com/themes/plugins/smm-api/';
        ...
        public function smm_add_variation_option( $loop, $variation_data, $variation ) { 
			            ...
			            checked( 'on', esc_attr( get_post_meta( $variation->ID, 'variable_smm_api', true ) ), true );
			            _e('/>');
		                _e( esc_html_e( 'SMM API', 'woocommerce' ));
		                
			            /* translators: Help tip to explain why and when you should check the 'SMM API' type of product in WooCommerce */
			            _e( wc_help_tip( esc_html__( 'Enable this option if the product is a smm-api', 'woocommerce' ) )); 
        ...
        public function smm_variation_settings_fields( $loop, $variation_data, $variation ) {
			    ...
			    $server_query = $wpdb->prepare( "SELECT smapi_p.post_title, smapi_p.ID FROM $wpdb->posts as smapi_p INNER JOIN " . $wpdb->prefix . "postmeta as smapi_pm ON ( smapi_p.ID = smapi_pm.post_id )
                WHERE 1=1
                AND smapi_p.post_type = %s
                AND smapi_pm.meta_key LIKE %s", $post_type, $parameter_key
                );
                $total_servers = $wpdb->query( $server_query );
                $my_smm_result  = $wpdb->get_results( $server_query, ARRAY_A );
                //Geting Item details for API ITEM LISTING
			        
                $item_query = $wpdb->prepare( "SELECT smapi_pm.* FROM " .$wpdb->prefix ."postmeta as smapi_pm INNER JOIN $wpdb->posts as smapi_p ON ( smapi_pm.post_id = '$api_server_list_options_saved' )
                WHERE 1=1 
                AND smapi_p.post_type = %s
                ...
                $smm_total_items = $wpdb->query( $item_query );
	            $smm_api_items_listing = $wpdb->get_results( $item_query, ARRAY_A);
	            //SERVER DETAILS
			    ...
			    foreach($smm_api_items_listing as $sub_item_result){
			            ...
			            $input_checkbox_default_item    = $sub_item_result['meta_key'];
    ...
    woocommerce_wp_select( array( // Text Field type
        ...
        'label'         => __( 'SMM API SERVICE ITEM', 'woocommerce' ),
        'wrapper_class' => 'form-row form-row-first',
        'class'         => 'select',
        'description'   => __( 'SMM SERVICE Item Loaded from SMM API ITEM LIST', 'woocommerce' ),
        'desc_tip'      => true,
    ...
    woocommerce_wp_select( array( // Text Field type
        ...
        'label'         => __( 'SMM SERVER PROVIDER', 'woocommerce' ),
        'wrapper_class' => 'form-row form-row-last',
        'description'   => __( 'SERVER Site Loaded from SMM API SERVER LIST', 'woocommerce' ),
        'desc_tip'      => true,
        'cbvalue'       => 'yes', // selected if same than 'value' (default value)
    ...
    _e('<div><span id="var_smapi_service_span_option_'. esc_attr( $loop ).'" style="float:left;display:block;">
    ...
    woocommerce_wp_text_input( array(
        ...
        'label'         => __( 'Customer Input Text Label', 'woocommerce' ),
        'description'   => __( 'Getting Customer Inputs', 'woocommerce' ),
        'desc_tip'      => true,
    ...
    woocommerce_wp_radio( array(
        ...
        'label'         => __('Locate Input Text Box', 'woocommerce'),
        'style'         => 'font-size: large;',
        'description'   => __( 'Selection of Input on page', 'woocommerce' ),
        'desc_tip'      => true,
         'value'        => $input_text_box_radio_saved,
        'options'       => array(
        'product'       => __('Use Custom Field at Product Page' , 'woocommerce'),
        'other'         => __('Use field From Other Plugin', 'woocommerce'),
        ...
        public function smm_add_custom_field_variation_data( $variations ) {
            $variations['var_smm_customer_input_field_label'] = '<div class="woocommerce_custom_field">Custom Field: <span>' . get_post_meta( $variations[ 'variation_id' ], 'var_smm_customer_input_field_label', true ) . '</span></div>';
            return $variations;
        }
		/**
		...
		*/
	    ...
	    foreach($smm_api_items_listing as $sub_item_result){
	       ...
	       ;
			...
			'placeholder'   => __( 'Enter URL OR Username', 'smm_api'),
...
_e( '</div>');
        ...
        public function add_custom_fields_for_single_products() {
            ...
            ?>
            <h3 class="smapi_price_is_per"><?php _e('Subscription Settings','smm-api') ?></h3>
            <div class="options_group show_if_simple show_if_variable">
                <p class="form-field smapi_price_is_per">
                    <label for="_smapi_price_is_per"><?php _e( 'Price is per', 'smm-api' ); ?></label>
                ...
                <p class="form-field smapi_max_length">
                    <label for="_smapi_max_length"><?php _e( 'Max length:', 'smm-api' ); ?>
                        <a href="#" class="tips" data-tip="<?php _e( 'Leave it empty for unlimited subscription', 'smm-api' ) ?>"> [?]</a></label>
                    ...
                    <span class="description"><span><?php _e( $time_opt = ( $_smapi_price_time_option ) ? esc_html($_smapi_price_time_option) : 'days') ?></span> <?php printf(__('(Max: <span class="max-l">%d</span>)', 'smm-api'), esc_html($max_lengths[$time_opt])) ?></span>
                </p>
...
<?php
	    ...
	    public function save_custom_fields_for_single_products( $post_id, $post ) {
		    ...
		    if ( isset( $_POST['_smapi_price_time_option'] ) && isset( $_POST['_smapi_max_length'] ) ) {
			    $max_length                = smapi_validate_max_length( $_POST['_smapi_max_length'], $_POST['_smapi_price_time_option'] );
		    ...
		    if ( isset( $_POST['_smapi_service_id_option'] ) ) {
				...
				$api_item_list_options_saved = $_POST['_smapi_service_id_option'];
        ...
        GROUP BY smapi_pm.meta_id
        ", $this->post_type, $api_item_list_options_saved
        );
		...
		foreach($smm_api_items_listing as $sub_item_result){
		   ...
		   $args['smm_custom_text_field_title'] =   sanitize_text_field($_POST['smm_custom_text_field_title']);
		    ...
		    $args['_smapi_api'] = isset( $_POST['_smapi_api'] ) && !empty($args['smm_custom_text_field_title']) ? 'yes' : 'no';
            }
        ...
        public function enqueue_styles_scripts() {
			...
			wp_enqueue_style( 'smms_smapi_select_dataTables', SMMS_SMAPI_ASSETS_URL . '/css/select.dataTables.css', SMMS_SMAPI_VERSION );
            ...
            wp_enqueue_script( 'jquery-blockui', SMMS_SMAPI_ASSETS_URL . '/js/jquery.blockUI.min.js', array( 'jquery' ), false, true );
            ...
            wp_enqueue_script( 'jquery-smm-script', SMMS_SMAPI_ASSETS_URL . '/js/smm-script.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'jquery-datatables', SMMS_SMAPI_ASSETS_URL . '/js/datatables.js', array( 'jquery' ), false, true );
			wp_enqueue_script( 'jquery-dataTables-select', SMMS_SMAPI_ASSETS_URL . '/js/dataTables.select.js', array( 'jquery' ), false, true );
			if ( defined( 'SMM_API_PREMIUM' ) )
			wp_enqueue_script( 'jquery-smm-premium',  '/wp-content/plugins/smm-api-premium/smm-premium.js', array( 'jquery' ), false, true );
        }
        ...
        public function register_panel() {
	        ...
	        if( smms_check_privacy_enabled() ){
		        $admin_tabs['privacy'] = __( 'Privacy', 'smm-api' );
	        }
            $admin_tabs['premium'] =  __( apply_filters('smm_premium_tab_label', 'Free Version'), 'smm-api' );

            $args = array(
                'create_menu_page' => true,
                'parent_slug'      => '',
                'page_title'       => 'SMMS API & WooCommerce Subscriptions',
        ...
        public function get_panel_sidebar_link() {
            return array(
                array(
                    'url'   => $this->doc_url,
                    'title' => __( 'Plugin Documentation', 'smm-api' )
                ),
                
                array(
                    'url'   => 'https://wordpress.org/support/plugin/smm-api',
                    'title' => __( 'WordPress support forum', 'smm-api' )
                ),
                array(
                    'url'   => $this->doc_url . '/changelog-free',
                    'title' => sprintf( __( 'Changelog (current version %s)', 'smm-api' ), SMMS_SMAPI_VERSION )
                ),
            );
        }
